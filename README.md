...
| 버전 | 날짜 | 주요 변경 사항 |
|---|---|---|
| 0.9.0 | 2026-09-25 | • 평가 신뢰성: 실행 환경 점검·환경 지문(run_manifest.environment), 대조군 CONTRADICTED·A/B 판정 오탐 집계, 결함 유형↔finding 유형 대응표, 모델 의견 점수는 실제 호출 시만, 블라인드 비교에 오탐 후보 목록, 환경·채점 방식이 다르면 '회귀 0건' 결론 금지<br>• 호증 표기 문법 파서(두 단 목록·쉼표·가운뎃점·가지번호 범위·피고인 증·증 제N호), 무작위 120건 왕복 시험, 대조군 결번·목록 누락 오탐 제거<br>• 법리 판정 극성(부정·판례 전달·상대방 주장·가정) 판별, 형사 규칙 2개와 전칭 일반화의 표현 변형 대응<br>• 증거표 열 구조 식별, ₩ 병기 대조, 작성명의 성 불일치, 비율(%) 재계산과 원인(합계) 연결, 같은 합계 판정 병합<br>• 사실 양태(확정·예정·가정·상대방 주장)와 대조 확대(피해액·편취금·변제액·합의금, 계산서 합계↔청구취지, Lt./Rt. 부위)<br>• 인젝션 경로: 클리핑 숨김(CLIPPED_OUT), 북마크 이어 읽기, PageLabels, 검증 억제 문형<br>• 본문·OCR 글의 달력에 없는 날짜(OCR 숫자 오인식 보정, OCR 신뢰도를 등급에 반영) (rule 2026.09.25.5) |
