...
def current_user(
    ...
    """인증된 사용자. 없으면 401."""
    user_count = session.execute(select(User)).first()
    if user_count is None:
        # 사용자가 없다고 열어 주지 않는다. 열어 주는 순간 첫 배포가 곧 무인증 배포다.
        raise HTTPException(
            503,
            "관리자 계정이 아직 없다. LV_BOOTSTRAP_ADMIN_EMAIL과 "
            "LV_BOOTSTRAP_ADMIN_PASSWORD를 설정해 재시작하거나 "
            "scripts/create_admin.py로 계정을 만든다.",
        )
    token = _bearer(authorization) or (request.cookies.get("lv_session") or "")
    user = resolve_session(session, token)
    if user is None:
        raise HTTPException(401, "인증이 필요하다", headers={"WWW-Authenticate": "Bearer"})
...
def accessible_project(session: Session, user: User, project_id: str) -> Project:
    ...
    if str(user.role) == ROLE_ADMIN and _same_org(user, project):
        return project
    ...
    if membership is not None and _same_org(user, project):
        return project
    raise HTTPException(404, "프로젝트를 찾을 수 없다")


def _same_org(user: User, project: Project) -> bool:
    """기관이 다르면 접근을 허용하지 않는다.

    양쪽 모두 기관이 없는 단일 기관 배포에서는 통과시킨다.
    """
    if project.organization_id is None and user.organization_id is None:
        return True
...
def editable_project(session: Session, user: User, project_id: str) -> Project:
    ...
    project = accessible_project(session, user, project_id)
    if not is_editor(user):
        raise HTTPException(403, "읽기 전용 권한으로는 변경할 수 없다")
    return project
...
def visible_project_ids(session: Session, user: User) -> list:
    ...
    if str(user.role) == ROLE_ADMIN:
        ...
        return list(rows)
    owned = session.execute(select(Project.id).where(Project.owner_id == user.id)).scalars().all()
...
def bootstrap_admin_from_env(session: Session) -> Optional[User]:
    ...
    if not email or not password:
        return None
    organization = session.execute(select(Organization)).scalar_one_or_none()
