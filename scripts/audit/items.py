...
ITEMS: List[Dict[str, Any]] = [
    ...
    {"id": "§3-6", "title": "AI 잔재 패턴(사전 분리)", "code": "verification_engine/ai_residue.py, config/ai_residue_patterns.yaml",
     "call": ("verification_engine/ai_document_detector.py", "scan_residue"), "tests": "test_v4_ai_residue.py",
     "family": lambda f: str(f.type) == "AI_RESPONSE_RESIDUE",
     "defects": [D("§3-6a", "챗봇 맺음말", lambda f: str(f.type) == "AI_RESPONSE_RESIDUE" and "맺음말" in title(f)),
                 D("§3-6b", "출처 토큰", lambda f: str(f.type) == "AI_RESPONSE_RESIDUE" and "출처" in title(f)),
                 D("§3-6c", "렌더링되지 않은 마크다운 표", lambda f: str(f.type) == "AI_RESPONSE_RESIDUE" and "마크다운" in title(f))]},
    ...
    {"id": "P6", "title": "AI 판별 집계(관여 여부·범위 분리, 약한 신호는 참고 부록)", "code": "verification_engine/ai_document_detector.py",
     ...
     "defects": [D("P6-1", "민사(AI 잔재 문서)에 AI 관여 근거 제시", lambda f: str(f.type) in {"AI_AUTHORSHIP_LIKELY", "AI_RESPONSE_RESIDUE"})],
