...
## 실제 사건자료를 다루기 전 — 저장 시 암호화

기본값은 **꺼짐**이다. 켜 두고 키를 잃으면 자료를 복구할 수 없으므로 자동으로 켜지 않는다.

| 환경변수 | 값 | 의미 |
| --- | --- | --- |
| `LV_STORAGE_ENCRYPTION` | `on` | 업로드 원본과 보고서 산출물을 봉투 암호화해 보관 |
| `LV_VAULT_KEY_PROVIDER` | `env` / `file` / `aws-kms` | 키 공급자 |

키는 `pii_engine`의 KeyProvider에서 오며 **자료 디스크에 두지 않는다.** `env`는
`LV_PSEUDONYM_SECRET` 또는 `LV_VAULT_KEYS`를 쓴다. 관리형 플랫폼이 이 값을 자동
생성했다면, 재생성되는 순간 기존 자료를 읽을 수 없으므로 **값을 따로 보관해야 한다.**

암호화를 켜기 전에 저장된 평문 자료는 계속 읽힌다. 봉투 표지가 없으면 평문으로
취급한다. 따라서 운영 중에 켤 수 있고, 이후 저장분부터 암호화된다.

사건별로 다른 키를 쓴다. 한 사건의 키로 다른 사건의 파일을 열 수 없다.

**가려지지 않는 구간.** 파서가 파일 경로를 요구하므로 처리 중에는 평문이 임시
디렉터리에 풀린다. 이 구간은 암호화로 가려지지 않는다. 이 사실을 전제로
디스크 접근 권한과 스냅샷 보관 정책을 함께 검토해야 한다.

상태는 `/api/diagnostics`의 `capabilities.storage_encryption.enabled`와 진단 화면의
"저장 시 암호화" 항목에서 확인한다.

## 배포 직후 반드시 확인 — 저장소 내구성

관리자로 `/api/diagnostics`를 열어 `capabilities.durability.verdict`를 본다.

| 값 | 의미 |
|---|---|
| `DURABLE` | 외부 데이터베이스·객체 저장소를 쓴다 |
| `UNKNOWN` | 경로가 영구 디스크인지 프로세스 안에서 확인할 수 없다. **안전하다는 뜻이 아니다** |
| `AT_RISK` | 재시작·재배포하면 저장된 내용이 사라진다 |

`AT_RISK`이면 기동 로그에도 경고가 남는다. 무엇이 사라지는지(`at_risk`)와 조치(`remedy`)가
함께 표시된다.

`LV_DATABASE_URL`을 지정하지 않으면 SQLite가 `{LV_DATA_DIR}/legal_verifier.db`에 생기고,
`LV_DATA_DIR`마저 없으면 애플리케이션 디렉터리 안에 생긴다. 관리형 플랫폼에서 그 경로는
재시작 시 초기화되므로 **로그인 세션, 프로젝트와 검증 기록, 업로드 원본, 감사기록이 모두
사라진다.** 화면은 정상으로 보이기 때문에 배포 직후 이 값을 확인하지 않으면 오래 발견되지
않는다.

실제로 이 상태의 배포에서, 분석 도중 반복해서 다시 로그인하라는 안내가 뜨는 증상이
나타났다. 원인은 세션 만료가 아니라 저장소가 재시작과 함께 초기화된 것이었다.

**기존 서비스 주의.** 저장소의 `render.yaml`을 고쳐도 이미 만들어진 서비스의 환경변수는
바뀌지 않는다. 대시보드에서 직접 확인·설정해야 한다.
