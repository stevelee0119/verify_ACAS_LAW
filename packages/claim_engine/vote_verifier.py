# -*- coding: utf-8 -*-
"""표결 인원 산술 검사 (COUNT / VOTE_CONTRADICTION).

검사 범위: 한 의결 서술 안에서 '찬성 + 반대 (+ 기권)' 인원 합이 재적 인원을 넘는지만 본다(산술적으로 성립 불가).
검사하지 않는 것: 의사·의결 정족수 충족 여부(재적 과반 출석, 출석 과반 찬성 등), 근거 규정(정관·법령)의 정족수,
제척·기피·회피 위원 제외, 서면 표결·대리 표결 허용 여부. 따라서 이 결과를 '법적 정족수 검증'으로 읽으면 안 된다.
"""
