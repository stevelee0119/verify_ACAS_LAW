...
class FindingType(StrEnum):
    ...
    OCR_LOW_QUALITY = "OCR_LOW_QUALITY"                    # 쪽별 OCR 신뢰도·한글 비율 미달(추가지시 G2)
    ...
    # 증거 정합성(v2 R9·Phase 7)
    EVIDENCE_DATE_INVALID = "EVIDENCE_DATE_INVALID"          # 증거 작성일이 달력에 없는 날짜
    EVIDENCE_TIMELINE_INVERSION = "EVIDENCE_TIMELINE_INVERSION"  # 작성일이 대상 사건보다 앞서거나 제출 서면보다 뒤
    EVIDENCE_NUMBERING_GAP = "EVIDENCE_NUMBERING_GAP"        # 호증 번호 결번
    EVIDENCE_LIST_MISMATCH = "EVIDENCE_LIST_MISMATCH"        # 본문이 언급한 호증이 증거 목록에 없음
    EVIDENCE_PERSON_INCONSISTENT = "EVIDENCE_PERSON_INCONSISTENT"  # 같은 사람의 계급·소속 기재가 서로 다름
    EVIDENCE_FORM_DEFECT = "EVIDENCE_FORM_DEFECT"            # 서명 생략 등 형식 요건 결함
    EVIDENCE_PURPOSE_MISMATCH = "EVIDENCE_PURPOSE_MISMATCH"  # 입증취지가 서증 성격 범위를 벗어남(사람 확인)
    STATEMENT_BEYOND_PERCEPTION = "STATEMENT_BEYOND_PERCEPTION"  # 지각하지 못했다면서 단정
    CROSS_DOC_COPY = "CROSS_DOC_COPY"                        # 다른 서면의 문장을 그대로 옮김
    MODEL_FACT_REMARK = "MODEL_FACT_REMARK"                  # AI 판별 모델이 지적한 날짜·금액·기간 모순(결정론 재계산 대상)
    ...
    OVERCLAIM = "OVERCLAIM"
    UNSUPPORTED_GENERALIZATION = "UNSUPPORTED_GENERALIZATION"  # 근거 없는 전칭 법리 명제(추가지시 G4)
...
SPEC_DEFAULT_SEVERITY = {
    ...
    FindingType.UNSUPPORTED_GENERALIZATION: Severity.MEDIUM,
